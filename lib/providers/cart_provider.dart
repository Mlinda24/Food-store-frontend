...
class CartProvider extends ChangeNotifier {
  ...
  double get deliveryFee =>
      _calculatedDeliveryFee > 0 ? _calculatedDeliveryFee : 2000.0;
  ...
  // FIXED: Helper method to resolve image URLs with proper IP replacement using static methods
  String _getImageUrl(dynamic image) {
    if (image == null) return '';
    String imageStr = image.toString();
    if (imageStr.isEmpty) return '';

    // If it's already a full URL, clean it using static method
    if (imageStr.startsWith('http')) {
      return ApiService.cleanImageUrlStatic(imageStr);
    }

    // If it's a relative path starting with /media/
    if (imageStr.startsWith('/media/')) {
      return ApiService.cleanImageUrlStatic(
          '${ApiService.mediaBaseUrl}$imageStr');
    }

    // Default case
    return ApiService.cleanImageUrlStatic(
        '${ApiService.mediaBaseUrl}/media/$imageStr');
  }
  ...
  Future<void> _fetchRestaurantDetails(String restaurantId) async {
    ...
    try {
      ...
      // Get image using the fixed method
  ...
  Future<String?> _fetchMenuItemImage(
      String menuItemId, int restaurantId) async {
    // Check cache first
      ...
      for (var item in menuItems) {
        if (item['id']?.toString() == menuItemId) {
          ...
          if (item['image_url'] != null &&
            ...
            imageUrl = _getImageUrl(item['image_url']);
    ...
    try {
      ...
      if (data != null && data.containsKey('items') && data['items'] is List) {
        ...
        for (var itemData in itemsData) {
          ...
          double price =
              double.parse(itemData['menu_item_price']?.toString() ?? '0');
          int quantity = itemData['quantity'] ?? 1;
          String? imageUrl;

          // Try to get image from API first
          if (menuItemId.isNotEmpty && _restaurantId != null) {
            imageUrl = await _fetchMenuItemImage(
                menuItemId, int.parse(_restaurantId!));
          }

          // If still no image, check if itemData has direct image field
          if ((imageUrl == null || imageUrl.isEmpty) &&
              itemData['image'] != null) {
            imageUrl = _getImageUrl(itemData['image']);
          }

          // Final fallback - try to get from menu_item object if exists
          if ((imageUrl == null || imageUrl.isEmpty) &&
              itemData['menu_item'] != null) {
        ...
        print(
            '✅ Loaded ${_items.length} items, Subtotal: MK${subtotal.toStringAsFixed(0)}');
  ...
  Future<void> calculateDeliveryFee({
    ...
    try {
      ...
      _calculatedDeliveryFee =
          DeliveryFeeCalculator.calculateFee(distanceInMeters);
      ...
      print(
          '   Distance: ${DeliveryFeeCalculator.formatDistance(distanceInMeters)}');
  ...
  void setDeliveryFeeFromRestaurant({
    ...
    print(
        '📦 Delivery fee set from restaurant: MK${deliveryFee.toStringAsFixed(0)}');
    _safeNotify();
  }
  ...
  void setDeliveryInfo({
    ...
    _lastCalculatedFee = deliveryFee;
    print(
        '📦 Delivery info set: ${DeliveryFeeCalculator.formatDistance(distanceInMeters)} = MK${deliveryFee.toStringAsFixed(0)}');
    _safeNotify();
  }
  ...
  Future<bool> addItem(MenuItem menuItem,
      {String? restaurantId, String? restaurantName, int quantity = 1}) async {
    ...
    try {
      final result =
          await _apiService.addToCart(int.parse(menuItem.id), quantity);
  ...
  Future<bool> addItemWithQuantity(MenuItem menuItem, int quantity,
  ...
  Order createOrder(
      String userId, String deliveryAddress, String? instructions) {
  ...
  Map<String, dynamic> getDeliverySummary() {
    return {
      ...
      'delivery_fee_formatted':
          DeliveryFeeCalculator.formatFee(_calculatedDeliveryFee),
